...
import { getSnapClasses } from './demo-snap-controls.js';
...
function simulateDemoSnap(demoCanvas, demoCanvasObject) {
    ...
    try {
        const { SnapControl, SnapTargetControl } = getSnapClasses();
        if (!SnapControl || !SnapTargetControl) {
            console.error('Fabric not available or Snap classes not initialized');
            return;
        }
